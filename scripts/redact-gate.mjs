...
const textExtensions = new Set(['.css', '.drawio', '.excalidraw', '.html', '.js', '.json', '.md', '.mjs', '.sh', '.toml', '.txt', '.yaml', '.yml']);
...
export const patterns = Object.freeze([
  ...
  { id: 'public-topology', label: 'public topology address', regex: /\b(?:(?:public|load_balancer|jumphost|control_plane|worker_node)[a-z_-]*_ip|endpoint)[ \t]*[:=][ \t]*["']?(?!<|\$\{|10\.|127\.|192\.0\.2\.|198\.51\.100\.|203\.0\.113\.)(?:\d{1,3}\.){3}\d{1,3}\b/gi },
  { id: 'private-topology', label: 'private topology address', regex: /\b(?:(?:private|load_balancer|jumphost|control_plane|worker_node)[a-z_-]*_ip|endpoint)[ \t]*[:=][ \t]*["']?(?!<|\$\{)10\.(?:\d{1,3}\.){2}\d{1,3}\b/gi },
...
function repositoryFiles() {
  ...
  return [...new Set(result.stdout.split('\n').filter(Boolean))]
    .filter(path => textExtensions.has(extname(path)) && !excludedFixtures.has(path))
    .sort();
}
